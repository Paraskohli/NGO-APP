...
class Tabel2 extends JFrame implements papaclass
{
	JPanel pnl1,pnl2;
	...
	/*	if(con==null)
		...
		//=========panel-------------
			    ...
			    pnl2.add(print);*/
					//-------------label
					name_hint=new JLabel("School Name Hint");
					name_hint.setSize(160,25);
 ...
 void fetchidA()
	{
		...
		tname_hint.addItem("----Select ID----");
		  pst=con.prepareStatement("select School_Name from NGO_Distributions");
		  ResultSet rs=pst.executeQuery();
		while(rs.next())
		 { 
			String txt=rs.getString("School_Name");
	...
	{
		try {
			pst=con.prepareStatement("select * from NGO_Distributions");
	...
	{
		try {
			pst=con.prepareStatement("SELECT * FROM NGO_Distributions WHERE School_Name LIKE '%' ? '%'");
...
public class LoadAllDistributions 
{
